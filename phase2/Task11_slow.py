...
class Task11_slow:
    ...
    def get_label_representatives(self, matrix, m, n, feature_model : str , feature : int, label_name :str,label_id : int, latent_space : int = None, latent_semantic : str = None, labelled_images : dict = None ) -> list :
        ...
        # 2 cases
        imageids_in_label_set = self.labelled_images[label_name]
        possible_seeds = []
        for i in imageids_in_label_set:
            if i % 2 == 0: 
                possible_seeds.append(i//2)
        seeds_to_take = len(possible_seeds) // 10
        seeds = set()
        maxPPR = 0
        
        print(f'Optimally, we try to consider {seeds_to_take} images in label contributing to PPR out of {len(possible_seeds)}')
        for ii in range(seeds_to_take):
            cur_max = float('-inf')
            cur_max_seed = -1
            for i in possible_seeds:
                cur_seed_set = list(seeds)
                cur_seed_set.append(i)
                
                # call pagerank, get max value consider and add that
                rankings = self.pagerankcall(matrix, cur_seed_set, m, n, label_name)
                cur_score_max = max(rankings[1])
                if cur_max < cur_score_max:
                    cur_max = cur_score_max
                    cur_max_seed = i
            seeds.add(cur_max_seed)
            maxPPR+=cur_max
            print(f'{cur_max_seed} choosen -> corresponds to {cur_max} with total: {maxPPR}')
        
        seeds = list(seeds)    
        
        print('#### Seed used ####: ')
        for i in seeds:
            print(f'Image Id: {i*2}')
        print('###################: ')    
        return seeds
    ...
    def runTask11(self):
        ...
        match option :
            ...
            case 1 : 
                ...
                seeds = self.get_label_representatives( matrix, m, n, feature_model, feature, label_name, label_id)
            ...
            case 2 : 
                ...
                if file == None :
                    ...
                    if ls_option == 3 :
                        ...
                        seeds = self.get_label_representatives( matrix, m, n, feature_model, fs_option, label_name, label_id, ls_option, d_reduction, self.labelled_images )
